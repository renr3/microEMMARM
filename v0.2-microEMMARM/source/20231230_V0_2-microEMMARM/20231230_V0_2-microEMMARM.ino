/*
  ...
  This is the source code for th microEMM-ARM system, v.0.2
  It has minor improvements in relation to the previous releases, and it is the first version that was implemented to work with the "EMM-ARM post-process" software
  It was developed to be run in an ATMega328p microcontroller (or in an Arduino UNO or compatible board) powered system.
  Refer to the GitHib repository for more information on how to assemble the system.
  The provided libraries in the GitHub pages are tested to work with the system, any new versions are not guarenteed to work due to retrocompatibility matters (they might, we just did not tested)
*/
...
byte passToSD = 0; //Parameter that 
//==============
//IMPORTANT INFO:
...
//      The function continuousAlert() below is the function performed when ADS1115 raises an ISR flag.
...
void continuousAlert() {
  ...
  //==============
  //IMPORTANT INFO:
  //      IAmRunning is a static local variable. The first time continuousAlert() is run, IAmRunning is initialized, but it is never initialized again
  //    and its value is preserved between different calls of continuousAlert().
  //      Check https://techexplorations.com/guides/arduino/programming/static/ for more info on static variables.
  //==============
  if (IAmRunning) return; //This will finish this run of continuousAlert() if it was called on top of other continuousAlert() already runnin,
                          //so to mitigate recursive calls that could freeze the system. 
  IAmRunning = true; //Indicates the system that a continuousAlert() is being run.
  interrupts(); //Enable interrupts so I2C and communication to external ADS (ADS1115 module) can work.
  adcX = ads.getValue(); //Gets the value from ADS1115. Remember that we are here just because ADS1115 raised an ISR flag saying a new value was ready to be read.
  passToSD = 1; //Signalizes the system that a new measurement has been read and is ready to be saved to log file
  IAmRunning = false; //Indicates the system that the on-going continuousAlert() has finished. 
}
...
void setup() {
  ...
  */
  ...
  //the others for which we need to map from sysConfig parameter to a real value
...
void loop() {
  ...
  for (currentChannel = 1; currentChannel <= sysConfig.numberOfActiveChannelsOption; currentChannel++) {
    ...
    */
    ...
    adcX = 0;
    attachInterrupt(digitalPinToInterrupt(alertPin), continuousAlert, RISING);
    while (millis() - startTime < samplingDuration * 1000) {
      if (passToSD == 1) {
        passToSD = 0;
        //samplingStamp=millis();
        currentOpenFile.write((const uint8_t *)&adcX, sizeof(adcX));
...
void checkSysConfigConsistency() {
  ...
  if (sysConfig.tempHumSensorOption < 1 || sysConfig.tempHumSensorOption > 2) {
    sysConfig.tempHumSensorOption = tempHumSensorOption_Default;
  }
